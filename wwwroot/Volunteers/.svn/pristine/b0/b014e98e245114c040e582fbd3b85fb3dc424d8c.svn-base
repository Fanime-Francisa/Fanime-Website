...
public partial class SettingsDefault : System.Web.UI.Page
{
    ...
    protected void ClearDatabaseBtn_Click(object sender, EventArgs e)
    {
        WebToolbox wt = new WebToolbox();

        try
        {
            ClearDatabaseDataSource.DeleteCommand = "DELETE FROM Confirmations;";
            ClearDatabaseDataSource.Delete();
            ClearDatabaseDataSource.DeleteCommand = "DELETE FROM TaskVolunteers;";
            ClearDatabaseDataSource.Delete();
            ClearDatabaseDataSource.DeleteCommand = "DELETE FROM Volunteers;";
            ClearDatabaseDataSource.Delete();
            ClearDatabaseDataSource.DeleteCommand = "DELETE FROM Tasks;";
            ClearDatabaseDataSource.Delete();
        }
        catch (Exception exc)
        {
            wt.LogError(exc.Message, "Error in Settings Page - Clearing the database", Page);
        }
    }
    protected void ExportDatabaseBtn_Click(object sender, EventArgs e)
    {
        Page.ClientScript.RegisterStartupScript(this.GetType(), "popup", "window.open('ReimbursementExport.aspx')", true);
    }
}